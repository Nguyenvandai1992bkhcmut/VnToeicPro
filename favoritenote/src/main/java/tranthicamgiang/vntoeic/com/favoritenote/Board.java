...
public class Board {
    ...
    public static final int BOARD_WIDTH =  7 * Note.NORMAL_SIZE + 8 * CommonDimen.DIVIDE;

    public static final int BOARD_HEIGHT_NORMAL = Utils.dpToPx(50);

    public static final int BOARD_HEIGHT_MINIMAL = Utils.dpToPx(38);

    public static final int BOARD_BOTTOM = (CommonDimen.HEIGH_VIEW_REACTION  + BOARD_HEIGHT_NORMAL)/ 2;
    ...
    public void setCoordinate(float centerX, float centerY) {
        boardX = centerX - BOARD_WIDTH / 2;
    }

    public void draw(Canvas canvas) {
        float radius = mCurrentHeight / 2;

        RectF board = new RectF(boardX, mCurrentY, boardX + BOARD_WIDTH, mCurrentY + mCurrentHeight);
