     ...
     vector<Part4*>result;
        while (sqlite3_step(stmt) == SQLITE_ROW){
              ...
              //
                ...
                Part4 *part4= new Part4(id,token1,script1,q1,a1,b1,c1,d1,sol1,q2,a2,b2,c2,d2,sol2,q3,a3,b3,c3,d3,sol3,level,time);
...
Part4* SqlitePart4::searchPart4Id(int id1){
     stringstream ss2;
     ss2 << id1;
     string id = ss2.str();
     string sql = "select * from part4 where part4_id="+id;
     if(sqlite3_prepare_v2((this->db),sql.c_str(), -1, &(this->stmt), NULL) != SQLITE_OK){
         return NULL;
     }
     vector<Part4*>result = funsearchPart() ;
     if(result.size()==0)return NULL;
     else return result.at(0);
}

vector<Part4*>SqlitePart4::randomPart4(int number1){
    stringstream ss2;
         ss2 << number1;
         ...
         string sql = "select * from part4 order by `time` limit "+ number;
         if(sqlite3_prepare_v2((this->db),sql.c_str(), -1, &(this->stmt), NULL) != SQLITE_OK){
             vector<Part4*>re;
             return re;
         }
...
vector<Part4*>SqlitePart4::randomPart4Subject(int subject1 ,int number1){
    stringstream ss2;
         ss2 << number1;
         string number = ss2.str();
    stringstream ss3;
         ...
         string sql = "select * from part4  where( part4_id in ( select part4_subject.part4_id from part4_subject where part4_subject.subject_id="+subject+")) order by `time` asc limit "+ number;
         if(sqlite3_prepare_v2((this->db),sql.c_str(), -1, &(this->stmt), NULL) != SQLITE_OK){
             vector<Part4*>re;
             /*
...
vector<Part4*>SqlitePart4::searchPart4Favorite(){


         string sql = "select * from part4 where part4_id  in (select id from part4_favorite)";
         if(sqlite3_prepare_v2((this->db),sql.c_str(), -1, &(this->stmt), NULL) != SQLITE_OK){
             vector<Part4*>re;
             /*
             const char * xx = sqlite3_errmsg(this->db);
